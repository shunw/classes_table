...
# the logic to choose the classes
- [x] prefer 为true的 (priority, category) 为key 索引各条 class schedule 1
...
- [x] check two dict, then add two dic, one category dict to limit the class category. And the other is the day dict, to limit the class quantity for one day
- [x] to the check each iteration.  
    - for one specific day, if there already exists two classes, then skip;
    - For one week, if the accumulated class category already beyond 2, then skip. 
- [] check block item, if the block item is block_downtown_area is True, then the class location should choose the downtown first. And the class time should choose after 12:00 and before the walking start --- all these are done in the data.py
    - [] Walk through the class location, which is downtown. then, check if there is any proper class there
